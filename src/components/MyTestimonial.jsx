...
const MyTestimonial = () => {
  ...
  useEffect(() => {
    const fetchTestimonial = async () => {
      try {
        const res = await fetch(`${BASE_URL}/recruiter/testimonial`, {
    ...
    <div className="max-w-2xl mx-auto py-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-900 rounded-2xl p-6 border border-gray-200 dark:border-gray-800 space-y-5">
          ...
          <div className="flex gap-1">
            {[1, 2, 3, 4, 5].map((star) => (
              <button key={star} type="button" onClick={() => setForm({ ...form, rating: star })} className="p-1">
                <Star size={28} className={`transition-colors ${star <= form.rating ? 'fill-theme_color text-theme_color' : 'text-gray-300 dark:text-gray-600'}`} />
              </button>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <input type="text" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} placeholder="e.g. HR Manager"
        ...
        <div>
          ...
          <input type="text" value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} placeholder="e.g. DAMAC Properties"
            className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-theme_color" />
        </div>

        <div>
          <label className="block text-sm font-semibold text-gray-900 dark:text-white mb-2">Your Testimonial *</label>
          <textarea value={form.text} onChange={(e) => setForm({ ...form, text: e.target.value })} placeholder="Share your experience hiring through HireGeniX..." rows={5}
