// src/components/recruiter/SuspendJob.jsx
...
const SuspendJob = ({ job, isOpen, onClose, setReload }) => {
  ...
  const handleSuspend = async () => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/recruiter/job/${job.id}/suspend`,
        {
          method: "PATCH",
          headers: {
      ...
      if (!response.ok) {
        ErrorMessage(data.message || "Failed to Suspend job");
        throw new Error(data.message || "Failed to Suspend job");
      }

      if (data.status === "SUCCESS") {
        successMessage(data?.message || "Job Suspended Successfully");
        setReload((prev) => !prev);
        onClose();
      ...
      setError(err.message || "An error occurred while Suspending the job");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    <Modal
      ...
      title={`Suspend Job`}
      subtitle="Your job posting Will be suspended and invisible to candidates upon Suspending"
      size="lg"
    >
      {/* Job Details Summary */}
        ...
        <div className="flex gap-3">
          ...
          <div className="flex-1">
            <h4 className="font-semibold text-amber-900 dark:text-amber-300 mb-1">
              Ready to suspend job?
            </h4>
            <p className="text-sm text-amber-800 dark:text-amber-400">
              Suspending this job will make it invisible to all candidates on
              the platform and moving it to DRAFT. Make sure you are certain of
              this action before proceeding.
            </p>
          </div>
        </div>
      </div>
