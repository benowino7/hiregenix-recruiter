// src/components/recruiter/UnSuspendJob.jsx
...
const UnSuspendJob = ({ job, isOpen, onClose, setReload }) => {
  ...
  const handleSuspend = async () => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/recruiter/job/${job.id}/unsuspend`,
        {
          method: "PATCH",
          headers: {
      ...
      if (!response.ok) {
        ErrorMessage(data.message || "Failed to UnSuspend job");
        throw new Error(data.message || "Failed to UnSuspend job");
      }

      if (data.status === "SUCCESS") {
        successMessage(data?.message || "Job UnSuspended Successfully");
        setReload((prev) => !prev);
        onClose();
      ...
      setError(err.message || "An error occurred while UnSuspending the job");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    <Modal
      ...
      title={`UnSuspend Job`}
      subtitle="Your job posting Will be unsuspended and sent to DRAFT for publishing to candidates"
      size="lg"
    >
      {/* Job Details Summary */}
        ...
        <div className="flex gap-3">
          ...
          <div className="flex-1">
            <h4 className="font-semibold text-amber-900 dark:text-amber-300 mb-1">
              Ready to unsuspend job?
            </h4>
            <p className="text-sm text-amber-800 dark:text-amber-400">
              Unsuspending this job will make it invisible to all candidates on
              the platform by sending it to DRAFT which you can later publish to
              candidates on need basis. Make sure you are certain of this action
              before proceeding.
            </p>
          </div>
        </div>
